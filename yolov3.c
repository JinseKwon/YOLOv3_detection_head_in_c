...
int run_inference(OrtSession* session, const ORTCHAR_T* input_file, int img_sz) {
    ...
    // OrtValue* output_tensors[5];
    // output_tensors[0] = NULL;
    // output_tensors[1] = NULL;
    // output_tensors[2] = NULL;
    // output_tensors[3] = NULL;
    // output_tensors[4] = NULL;
    ...
    for(int n = 0; n<5; n++){
      ...
      printf("output : dims[%d] [1][2][3][4] : %ld,%ld,%ld,%ld\n",n,dims[n][0],dims[n][1],dims[n][2],dims[n][3]);
      fflush(stdout);
    }
    ...
    int index = 0;
    for(int n = 2; n < 5; n++){
      index = dims[n][2] * dims[n][3];
      for(int c = 0; c < dims[n][1]; c++){
    ...
    //YOLO 실행  
    yolov3_det(out, (char*)input_file_p, img_sz);  
